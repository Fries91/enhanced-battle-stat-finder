...
// @version      1.0.1
// @description  Standalone war enemy stat finder with one-login, spies, estimates, and attack-result learning.
// @author       Fries91
...
(function () {
  ...
  GM_addStyle(`
    ...
    @media(max-width:760px){#ebsfPanel{width:calc(100vw - 6px);height:calc(100vh - 6px);border-radius:10px}.grid,.row{grid-template-columns:1fr}.target{grid-template-columns:1fr}.acts{justify-content:flex-start}}
  `);
  ...
  function boot(){
    ...
    r.innerHTML=`
      <div id="ebsfPanel">
        <div class="ebsfHead">
          <div><b>⚔️ Enhanced Battle Stat Finder</b><small>one-login • spies • BSP/FFS estimates • learning</small></div>
        ...
        <div class="ebsfTabs">
          ...
          <button class="ebsfTab" data-tab="learning">Learning</button>
          <button class="ebsfTab" data-tab="settings">Settings / ToS</button>
        </div>
        <div id="ebsfBody"></div>
      </div>`;
  ...
  function render(){ if(app.tab==='targets') targets(); if(app.tab==='spy') spy(); if(app.tab==='learning') learning(); if(app.tab==='settings') settings(); }

  function targets(){
    q('#ebsfBody').innerHTML=`<div class="grid"><div class="card"><h3>One Login</h3>
      <input id="key" class="inp" type="password" placeholder="Torn limited API key" value="${esc(app.key)}" style="margin-bottom:8px">
      <div class="row"><input id="total" class="inp" placeholder="Your effective total stats" value="${esc(app.total)}"><input id="enemyFaction" class="inp" placeholder="Enemy faction ID optional" value="${esc(app.enemyFaction)}"></div>
      <div class="row"><input id="tskeyTop" class="inp" type="password" placeholder="TornStats key optional" value="${esc(app.tornstats)}"><input id="yatakeyTop" class="inp" type="password" placeholder="YATA key optional" value="${esc(app.yata)}"></div>
      <div class="row"><button id="login" class="btn">Login / Save All</button><button id="scan" class="btn btn2">Scan Enemy</button></div>
      ${app.user?`<div class="note">Logged in as <b>${esc(app.user.name)}</b> [${app.user.user_id}]<br>Faction: ${esc(app.user.faction_name||'')} [${esc(app.user.faction_id||'')}]</div>`:'<div class="note">Not logged in yet.</div>'}
      <div class="msg ${app.msg?'show':''}">${esc(app.msg)}</div>
      <p class="note">Torn key is the main login. TornStats/YATA keys are optional and saved once for import buttons.</p>
  ...
  function spy(){
    ...
    q('#ebsfBody').innerHTML=`<div class="grid"><div class="card"><h3>Exact Spy</h3>
      ...
      <div class="card" style="margin-top:12px"><h3>TornStats / YATA Import</h3><div class="row"><input id="iid" class="inp" placeholder="Target ID" value="${t.user_id||''}"><input id="iname" class="inp" placeholder="Name" value="${esc(t.name||'')}"></div><div class="row"><button id="tsImport" class="btn btn2">Import TornStats</button><button id="yataImport" class="btn btn2">Import YATA</button></div><p class="note">TornStats key saves with one-login. YATA needs the backend YATA_API_BASE endpoint once confirmed.</p></div>`;
    q('#saveSpy').onclick=saveSpy;
    q('#saveEst').onclick=saveEst;
    q('#tsImport').onclick=tsImport;
    q('#yataImport').onclick=yataImport;
  }

  function learning(){
    const t=app.selected||{};
    q('#ebsfBody').innerHTML=`<div class="card"><h3>Attack Result Learning</h3><div class="row"><input id="lid" class="inp" placeholder="Target ID" value="${t.user_id||''}"><input id="lname" class="inp" placeholder="Target name" value="${esc(t.name||'')}"></div><input id="ltotal" class="inp" placeholder="Your effective total stats" value="${esc(app.total)}" style="margin-bottom:8px"><div class="row"><button class="btn" data-res="easy_win">Easy Win</button><button class="btn" data-res="close_win">Close Win</button></div><div class="row"><button class="btn btn2" data-res="close_loss">Close Loss</button><button class="btn btn2" data-res="hard_loss">Hard Loss</button></div><button class="btn btn2" data-res="could_not_hit">Could Not Hit</button><p class="note">Every result narrows the shared stat range over time.</p></div>`;
    q('#ebsfBody').querySelectorAll('[data-res]').forEach(x=>x.onclick=()=>saveResult(x.dataset.res));
  }

  function settings(){
    q('#ebsfBody').innerHTML=`<div class="grid"><div class="card"><h3>Saved Integrations</h3><input id="tskey" class="inp" type="password" placeholder="TornStats key" value="${esc(app.tornstats)}" style="margin-bottom:8px"><input id="yatakey" class="inp" type="password" placeholder="YATA key/token" value="${esc(app.yata)}" style="margin-bottom:8px"><button id="saveSet" class="btn">Save Settings</button></div><div class="card"><h3>ToS / Data Use</h3><p class="note">Use a Torn limited API key. This app stores your user ID, faction ID, shared enemy spy entries, manual estimates, and attack-result labels. Data is used to rank enemy war targets and improve stat estimates for your faction. Do not enter passwords. Do not submit spy or estimate data you are not allowed to share.</p><p class="note">BSP/FFS are treated as manual estimate sources unless you have a documented allowed API connector.</p></div></div>`;
    q('#saveSet').onclick=async()=>{
      ...
      msg('Settings saved for one-login use.');
    };
  }

  function actTarget(e){
    const id=Number(e.currentTarget.dataset.id);
    const m=app.members.find(x=>Number(x.user_id)===id)||{user_id:id,name:'Unknown'};
    app.selected=m;
    const a=e.currentTarget.dataset.act;
    if(a==='profile') location.href=`https://www.torn.com/profiles.php?XID=${id}`;
    if(a==='attack') location.href=`https://www.torn.com/loader.php?sid=attack&user2ID=${id}`;
    if(a==='spy'){app.tab='spy';setTabs();render();}
    if(a==='learn'){app.tab='learning';setTabs();render();}
  }

  async function login(){
    app.key=q('#key').value.trim();
    app.total=q('#total').value.trim();
    app.enemyFaction=q('#enemyFaction').value.trim();
    app.tornstats=(q('#tskeyTop')?.value||'').trim();
    app.yata=(q('#yatakeyTop')?.value||'').trim();
    ...
    if(r.ok){
      app.user=r.user;
      GM_setValue(S.user,JSON.stringify(r.user));
      await post('/api/settings/integrations',{user_id:app.user.user_id,tornstats_key:app.tornstats,yata_key:app.yata,share_attack_learning:true,share_spies:true});
      msg('Logged in and saved all keys for this app.');
  ...
  async function saveSpy(){
    ...
    msg(r.ok?'Spy saved.':'Spy failed: '+JSON.stringify(r.error||r));
  }
  ...
  async function saveEst(){
    ...
    msg(r.ok?'Estimate saved.':'Estimate failed: '+JSON.stringify(r.error||r));
  }
  ...
  async function tsImport(){
    ...
    msg(r.ok?'TornStats imported.':'TornStats failed: '+JSON.stringify(r.error||r));
  }

  async function yataImport(){
    const r=await post('/api/integrations/yata/import-user',{yata_key:app.yata,target_id:q('#iid').value,target_name:q('#iname').value,submitted_by:app.user?.user_id});
    msg(r.ok?'YATA imported.':'YATA failed: '+JSON.stringify(r.error||r));
  }

  async function saveResult(res){
    const r=await post('/api/attack/result',{attacker_id:app.user?.user_id,attacker_name:app.user?.name,attacker_total:q('#ltotal').value,target_id:q('#lid').value,target_name:q('#lname').value,result:res});
    msg(r.ok?'Learning result saved.':'Result failed: '+JSON.stringify(r.error||r));
  }
  ...
  function fmt(n){n=Number(n||0); if(!n)return'?'; if(n>=1e9)return(n/1e9).toFixed(2)+'b'; if(n>=1e6)return(n/1e6).toFixed(1)+'m'; if(n>=1e3)return(n/1e3).toFixed(1)+'k'; return String(Math.round(n));}
})();
